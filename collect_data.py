...
FIELDNAMES = ["asset", "platform", "alert_id", "alert_hash", "created_at", "severity", "tx_hash", "block", "name", "description", "type", "projects"]
...
class ContractEventTracker:
  def __init__(self, w3, db, address, abi, block_key, step):
    self.contract = w3.eth.contract(address=address, abi=abi)

    self.db = db
    self.block_key = block_key
    with db.begin() as txn:
      self.block = json.loads(txn.get(block_key))
  ...
  async def extract_addresses(self, entries):
    pass

  async def crawl_events(self, fromBlock, toBlock):
    pass
  ...
  async def find_addresses(self):
    if self.crawl_finished:
      ...
      with self.db.begin(write=True) as txn:
        txn.put(self.block_key, json.dumps(fromBlock).encode())
...
class Wyvern(ContractEventTracker):
  def __init__(self, w3, db):
    ...
    block_key = b"wyvern_block"
    step = 10_000
    super().__init__(w3, db, address, abi, block_key, step)
  ...
  async def crawl_events(self, fromBlock, toBlock):
    ...
    return crawl_filter.get_all_entries()

  async def poll_events(self):
    self.poll_filter.get_new_entries()

  def extract_addresses(self, entries):
    return entries


class Seaport(ContractEventTracker):
  def __init__(self, w3, db):
  ...
  async def poll_events(self):
    return self.poll_filter.get_new_entries()
...
class LooksRare(ContractEventTracker):
  def __init__(self, w3, db):
  ...
  async def crawl_events(self, fromBlock, toBlock):
    ...
    return ask_entries + bid_entries
...
class Rarible(ContractEventTracker):
  def __init__(self, w3, db):
...
class Foundation(ContractEventTracker):
  ...
  async def crawl_events(self, fromBlock, toBlock):
    ...
    return buy_entries + auction_entries + offer_entries

  async def poll_events(self):
    async def get_buy_entries(): return self.buy_poll_filter.get_new_entries()
    async def get_auction_entries(): return self.auction_poll_filter.get_new_entries()
    async def get_offer_entries(): return self.offer_poll_filter.get_new_entries()
...
async def query_alert_data(forta, source, addresses):
  ...
  def process_row(address, alert):
    if alert["projects"] == None:
      projects = []
    else:
      projects = list(map(lambda project: project["name"], alert["projects"]))
    ...
    return {
      ...
      "tx_hash": alert["source"]["transactionHash"],
      "block": int(alert["source"]["block"]["number"]),
      "name": alert["name"].replace("\00", ""),
      "description": alert["description"].replace("\00", ""),
      "type": alert["findingType"],
      "projects": projects,
    }
